...
def get_transform(opt, train=True, pretensor_transform=False):   #一系列的操作手段 resize还有数据增强（True）啥的
    transforms_list = []
...
def get_dataloader(opt, train=True,set_ISIC2019='Train', pretensor_transform=False):
    ...
    if opt.dataset=='ISIC2019':
        csv_path = '/home/s22_q/IBAQ/txt/cifar10_data.csv'
        root_path = '/home/s22_q/IBAQ/cifar10_images/'
        roo_path = '/home/s22_q/IBAQ/test_images/'
        if set_ISIC2019 == 'Train':
            dataset = CSVDataset(root=root_path, csv_file=csv_path, image_field='image', target_field='label',
                               transform=transform, add_extension='.png',
                               split='/home/s22_q/IBAQ/txt/train'+str(opt.split_idx)+'.txt')
        elif set_ISIC2019 == 'Val':
            dataset = CSVDataset(root=root_path, csv_file=csv_path, image_field='image', target_field='label',
                                 transform=transform, add_extension='.png',
                                 split='/home/s22_q/IBAQ/txt/validation' + str(opt.split_idx) + '.txt')
        elif set_ISIC2019 == 'Test':
            dataset=CSVDataset(root=roo_path, csv_file=csv_path, image_field='image', target_field='label',
                              transform=transform, add_extension='.png',
                              split='/home/s22_q/IBAQ/txt/test' + str(opt.split_idx) + '.txt')
        else:
    ...
    #根据传入的参数，创建了一个数据加载器对象dataloader，用于加载dataset中的数据。每个批次的大小由opt.bs指定，opt.num_workers指定了数据加载的并行进程数。
    # 数据加载器会在每个时期重新打乱数据，并且最后一个不完整的批次将会被丢弃。
    return dataloader
